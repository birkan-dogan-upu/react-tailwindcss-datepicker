...
const ItemTemplate = memo((props: ItemTemplateProps) => {
    ...
    const getClassName = useCallback(() => {
        const textColor = TEXT_COLOR["500"][primaryColor as keyof (typeof TEXT_COLOR)["500"]];
        const textColorHover = TEXT_COLOR.hover[primaryColor as keyof typeof TEXT_COLOR.hover];
        return `whitespace-nowrap w-1/2 md:w-1/3 xl:w-auto transition-all duration-300 hover:bg-gray-100 p-2 rounded cursor-pointer ${textColor} ${textColorHover}`;
    }, [primaryColor]);
...
const Shortcuts = () => {
    ...
    return shortcutOptions?.length ? (
        <div className="md:border-b mb-3 xl:mb-0 xl:border-r xl:border-b-0 border-gray-300 pr-2">
            <ul className="w-full tracking-wide flex flex-wrap xl:flex-col pb-1 xl:pb-0">
